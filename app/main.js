...
class Ball {
    ...
    draw() {
        /** @type {CanvasRenderingContext2D}  */
        let ball = new Path2D();
        ball.arc(this.x, this.y, this.radius, 0, Math.PI * 2, true);
    ...
    isBorderColliding() {
        if ((this.x + this.xSpeed) <= this.diameter || (canvas.width - this.x - this.xSpeed) <= this.diameter) this.changeXspeed();
        if ((this.y + this.ySpeed) <= this.diameter || (canvas.height - this.y - this.ySpeed) <= this.diameter) this.changeYspeed();
    }
    changeYspeed() {
...
class Game {
    ...
    createBricks(topLeft, topRight, bottomLeft, bottomRight){
        let bricksColors = [[64, 100, 50], [117, 100, 15], [298, 100, 50], [30, 100, 50], [0, 0, 99]]; // colors in HSL format
        ...
        for (let index = 1; index <= 8; index++) {
            for (let Yindex = 1; Yindex <= 5; Yindex++) {
                ...
                } else if (brick.x > canvas.width / 2) {
